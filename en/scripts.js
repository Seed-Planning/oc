  ...
  const setupSubnavToggles = () => {
    navSubnavItems.forEach((item, index) => {
      ...
      if (!button) {
        ...
        button.setAttribute('aria-label', `Toggle pages for ${link.textContent.trim()}`);
  ...
  if (carousel) {
    ...
    const buildDots = () => {
      ...
      pageStarts.forEach((pageStart, pageIndex) => {
        ...
        button.setAttribute('aria-label', `Patient profiles, page ${pageIndex + 1}`);
    ...
    const updateCarousel = () => {
      ...
      carousel.setAttribute('aria-label', `Profiles of Patients A to F. Currently showing from profile ${startIndex + 1}`);
    };
...
// English hint paragraphs can be longer than the original fixed-height panels.
// Measure their actual content while retaining the original CSS reveal animation.
(() => {
  'use strict';
  const cards = Array.from(document.querySelectorAll('.hint-card'));
  if (!cards.length) return;
  const measure = (card) => {
    const text = card.querySelector('p');
    if (!text) return;
    const height = `${Math.ceil(text.scrollHeight) + 4}px`;
    if (card.style.getPropertyValue('--hint-content-height') !== height) {
      card.style.setProperty('--hint-content-height', height);
    }
  };
  const measureAll = () => cards.forEach(measure);
  cards.forEach((card) => {
    card.addEventListener('pointerenter', () => requestAnimationFrame(() => measure(card)));
    card.addEventListener('focusin', () => requestAnimationFrame(() => measure(card)));
  });
  if ('ResizeObserver' in window) {
    const widths = new WeakMap();
    const observer = new ResizeObserver((entries) => {
      entries.forEach(({target, contentRect}) => {
        const width = Math.round(contentRect.width * 10) / 10;
        if (widths.get(target) === width) return;
        widths.set(target, width);
        const card = target.closest('.hint-card');
        if (card) measure(card);
      });
    });
    cards.forEach((card) => {
      const text = card.querySelector('p');
